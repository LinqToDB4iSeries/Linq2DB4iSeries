...
namespace Tests.DataProvider
{
	public partial class DB2iSeriesTests
	{
		...
		public void TestObject3()
		{
			...
			{
				Assert.That(conn.Execute<byte[]>("VALUES (@p)", new { p = arr1 }), Is.EqualTo(arr1));
				Assert.That(conn.Execute<byte[]>("VALUES (@p)", new { p = arr2 }), Is.EqualTo(arr2));
			}
		}
		...
		public void TestObject4()
		{
			using (var conn = new DataConnection())
			{
				Assert.That(conn.Execute<int>("VALUES (@p)", new { p = 1 }), Is.EqualTo(1));
			}
		}
