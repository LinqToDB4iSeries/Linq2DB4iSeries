...
namespace Tests.Linq
{
	...
	public class AssociationTests : TestBase
	{
		...
		public void GroupBy3(string context)
		{
			using (var db = GetDataContext(context))
			{
				var val1 = from p in Parent group p by p.Types.DateTimeValue.Year into g select g.Key;
				var val2 = from p in db.Parent group p by p.Types.DateTimeValue.Year into g select g.Key;
				AreEqual(val1, val2);
			}
		}
