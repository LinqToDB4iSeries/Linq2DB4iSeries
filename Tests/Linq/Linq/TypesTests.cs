...
namespace Tests.Linq
{
	...
	public class TypesTests : TestBase
	{
		...
		public void DateTime1(string context)
		{
			...
			using (var db = GetDataContext(context))
			{
				var expected = from t in Types2 where t.DateTimeValue.Value.Date >  dt.Value.Date select t;
				var actual = from t in db.Types2 where t.DateTimeValue.Value.Date > dt.Value.Date select t;
		...
		public void Unicode(string context)
		{
			using (var db = GetDataContext(context))
			{
				...
				try
				{
					...
					var byId = db.Person.Single(p => p.ID == pid);
