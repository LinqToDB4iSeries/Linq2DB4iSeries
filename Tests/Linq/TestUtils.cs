    ...
    public static class TestUtils
    {
        ...
        public static string GetSchemaName(IDataContext db)
        {
            switch (GetContextName(db))
            {
                ...
                case TestProvName.SqlAzure:
                    return db.GetTable<LinqDataTypes>().Select(x => SchemaName()).First();
            }
        ...
        public static string GetDatabaseName(IDataContext db)
        {
            switch (GetContextName(db))
            {
                ...
                case ProviderName.SqlServer2014:
                case TestProvName.SqlAzure:
                    return db.GetTable<LinqDataTypes>().Select(x => DbName()).First();
                case ProviderName.Informix:
                    return db.GetTable<LinqDataTypes>().Select(x => DbInfo("dbname")).First();
            }
        ...
        public static bool ProviderNeedsTimeFix(this IDataContext db, string context)
        {
            if (context == "MySql" || context == "MySql.LinqService")
            {
                ...
                var version = db.GetTable<LinqDataTypes>().Select(x => MySqlVersion()).First();
                var match = new Regex(@"^\d+\.\d+.\d+").Match(version);
                if (match.Success)
                {
                    var versionParts = match.Value.Split('.').Select(x => int.Parse(x)).ToArray();
