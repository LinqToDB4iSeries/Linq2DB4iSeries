...
namespace LinqToDB.DataProvider.DB2iSeries
{
	static class StringBuilderExtensions
	{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static StringBuilder AppendSubstring(this StringBuilder sb, string @string, int index, int? length = null)
		{
			...
			if (length.HasValue && (length < 0 || index + length > @string.Length))
			...
			{
				sb.Append(@string[index + i]);
			}
#else
			sb.Append(@string.AsSpan(index, length.Value));
#endif
